...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState){
        ...
        TextView title=new TextView(this); title.setText("区域连点器 v1.4"); title.setTextSize(26); title.setGravity(Gravity.CENTER); root.addView(title,mw());
        TextView desc=new TextView(this); desc.setText("新增：\n• 中央橙色移动点：拖动即可移动整个点击区域\n• 四角橙色小手柄：连续拖拽改变区域宽高\n• 区域框本体仍永久触摸透传\n• 1~1000ms 请求间隔\n• 默认 100ms\n• 开始前 3 秒倒计时\n• 运行时按音量减立即停止\n• 连续 3 次手势取消或调度失败自动停止\n• 开机后服务只待命，不自动显示、不自动点击\n\n使用：开启无障碍服务 → 回到本页 → 显示控制器 → 拖中央点移动区域 / 拖四角缩放 → 选择速度 → 开始。"); desc.setTextSize(15); desc.setTextColor(Color.DKGRAY); LinearLayout.LayoutParams dlp=mw(); dlp.topMargin=dp(18); root.addView(desc,dlp);
        ...
        TextView note=new TextView(this); note.setText("1ms 是请求调度间隔，不等于实际 1000 次/秒。首次仍建议先从 100ms 测试，并先确认音量减急停正常。"); note.setTextSize(13); note.setTextColor(Color.rgb(160,70,35)); LinearLayout.LayoutParams nlp=mw(); nlp.topMargin=dp(18); root.addView(note,nlp);
        setContentView(root);
    }
